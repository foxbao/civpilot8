...
void SerialStream::open(void) {
  fd_ = ::open(device_name_.c_str(), O_RDWR | O_NOCTTY | O_NDELAY | O_NONBLOCK);
  if (fd_ < 0) {
    ...
    if (bytes_current_read < 0) {
      switch (errno) {
        ...
        case EIO:
          ...
          Disconnect();
          if (Connect()) {
            AERROR << "Reconnect " << device_name_ << " success.";
            bytes_current_read = 0;
          } else {
            AERROR << "Reconnect failed";
          }
...
size_t SerialStream::write(const uint8_t *data, size_t length) {
  if (!is_open_) {
    ...
    AERROR << "Connect " << device_name_ << " success.";
  }
...
SerialStream *SerialStream::CreateSerial(const char *device_name,
                                         uint32_t baud_rate,
                                         uint32_t timeout_usec) {
